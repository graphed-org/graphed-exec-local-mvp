...
def test_pooled_matches_driver_path_results_and_combine_count(n: int) -> None:
    ...
    pooled = ThreadExecutor(pooled_combines=True).run(plan)
    driver = ThreadExecutor().run(plan)
...
def test_combines_run_off_the_driver_thread() -> None:
    ...
    result = ThreadExecutor(max_workers=4, pooled_combines=True).run(plan)
    tids = result.value
...
def test_combines_run_in_worker_processes() -> None:
    ...
    result = ProcessExecutor(max_workers=2, pooled_combines=True).run(plan)
    pids = result.value
...
def test_process_pool_pooled_matches_thread_pool_pooled() -> None:
    ...
    a = ProcessExecutor(max_workers=2, pooled_combines=True).run(plan)
    b = ThreadExecutor(pooled_combines=True).run(plan)
...
def test_straggler_does_not_block_other_subtrees() -> None:
    ...
    result = ThreadExecutor(max_workers=4, on_combine=seen.append, pooled_combines=True).run(plan)
